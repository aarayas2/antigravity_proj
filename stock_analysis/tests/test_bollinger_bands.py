...
from strategy.BollingerBands import (  # pylint: disable=wrong-import-position, import-error
    add_traces,
    apply_strategy,
    get_signals,
    needs_subplots,
...
def test_bollinger_bands_apply_strategy():
    """Test basic apply_strategy for Bollinger Bands."""
    np.random.seed(42)
...
def test_bollinger_bands_not_enough_data():
    """Test apply_strategy with insufficient data."""
    np.random.seed(42)
...
def test_bollinger_bands_get_signals_with_signals():
    """Test get_signals with valid buy/sell signals."""
...
def test_bollinger_bands_get_signals_no_signal_column():
    """Test get_signals when Signal column is missing."""
...
def test_bollinger_bands_get_signals_all_zero():
    """Test get_signals when all signals are zero."""
    dates = pd.date_range(start='2023-01-01', periods=3)
    ...
    assert len(buy_signals) == 0, \
        'Buy signals should be empty if there are no 1.0 signals.'
    assert len(sell_signals) == 0, \
        'Sell signals should be empty if there are no -1.0 signals.'


def test_needs_subplots():
    """Test if strategy needs subplots."""
    assert needs_subplots() is False


@patch('strategy.BollingerBands.go.Scatter')
def test_add_traces_with_subplots(_mock_scatter):
    """Test add_traces with subplots enabled."""
    fig = MagicMock()
...
@patch('strategy.BollingerBands.go.Scatter')
def test_add_traces_no_subplots(_mock_scatter):
    """Test add_traces without subplots."""
    fig = MagicMock()
    df = pd.DataFrame({'BBU_20_2.0_2.0': [100.0, 105.0], 'BBL_20_2.0_2.0':
        [90.0, 95.0], 'BBM_20_2.0_2.0': [95.0, 100.0]}, index=pd.date_range
        (start='2023-01-01', periods=2))
...
def test_add_traces_no_bbands():
    """Test add_traces with missing Bollinger Bands columns."""
    fig = MagicMock()
...
@patch('strategy.BollingerBands.go.Scatter')
def test_add_traces_with_bbu_column_main_row(_mock_scatter):
    ...
    _mock_scatter.return_value = mock_scatter_instance
    add_traces(fig, df, main_row=1, sub_row=None)
    assert fig.add_trace.call_count == 3
    for call in fig.add_trace.call_args_list:
        assert call[1]['row'] == 1
...
@patch('strategy.BollingerBands.go.Scatter')
def test_add_traces_with_bbu_column_no_main_row(_mock_scatter):
...
def test_apply_strategy_normal():
    ...
    assert result_df['Signal'].iloc[20] == 1.0, \
        'Expected buy signal on row 20 (price dropped below lower band).'
    assert result_df['Signal'].iloc[21] == -1.0, \
        'Expected sell signal on row 21 (price spiked above upper band).'
    assert result_df['Signal'].iloc[22] == 0.0, \
        'Expected no signal on row 22 (price within bands).'
...
def test_apply_strategy_insufficient_data():
    ...
    result_df = apply_strategy(df)
    assert 'BBL_20_2.0_2.0' not in result_df.columns, \
        'Should not add BBL column with insufficient data.'
    assert 'Signal' not in result_df.columns, \
        'Should not add Signal column with insufficient data.'
...
def test_apply_strategy_with_mock_df_15_lines():
    """Apply pandas-ta indicators to a mock df and assert output shape and signals."""
