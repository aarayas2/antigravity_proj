"""
Tests for the SMA strategy module.
"""
import os
import sys
...
from strategy.sma import apply_strategy, get_signals, needs_subplots, add_traces  # pylint: disable=wrong-import-position
...
def test_sma_apply_strategy():
    ...
    sma20_valid = result_df.loc[valid_mask, 'SMA_20']
    sma50_valid = result_df.loc[valid_mask, 'SMA_50']
    expected_signal = (sma20_valid > sma50_valid).astype(float)
...
def test_sma_add_traces_with_main_row(_mock_scatter):
...
@patch('strategy.sma.go.Scatter')
def test_sma_add_traces_no_main_row(_mock_scatter):
...
@patch('strategy.sma.go.Scatter')
def test_sma_add_traces_missing_columns(_mock_scatter):
