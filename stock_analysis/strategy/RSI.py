...
def apply_strategy(df: pd.DataFrame) -> pd.DataFrame:
    """Applies the RSI strategy to the dataframe."""
    df = df.copy()
...
def add_traces(fig, df_with_signals, main_row, sub_row):
    """Adds RSI traces to the given figure."""
    # pylint: disable=unused-argument
    fig.add_trace(go.Scatter(
        x=df_with_signals.index, y=df_with_signals['RSI_14'],
        line={'color': 'purple', 'width': 1.5}, name='RSI 14'
    ), row=sub_row, col=1)
